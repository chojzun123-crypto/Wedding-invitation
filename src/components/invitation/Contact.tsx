  ...
  return (
    <div className="flex items-center justify-between gap-2 py-3.5">
      <div className="min-w-0">
        ...
        <p className="whitespace-nowrap text-[15px] text-ink">{name}</p>
      </div>
      <div className="flex shrink-0 gap-1.5">
        <a
          href={`tel:${tel.replace(/-/g, "")}`}
          aria-label={`${name}에게 전화`}
          className="flex h-9 w-9 items-center justify-center rounded-full border border-line bg-paper text-point active:scale-95"
        >
        ...
        <a
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full border border-line bg-paper text-accent active:scale-95"
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
    ...
    <Section label="CONTACT" title="연락하기" className="bg-paper">
      <Reveal className="mx-auto max-w-card">
        <div className="grid gap-4 sm:grid-cols-2">
          {groom.length > 0 && (
            <div className="rounded-2xl border border-line bg-ivory/50 px-4 py-3">
